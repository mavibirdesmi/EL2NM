...
def get_dataset_noise_visualization(dataset_arg, filepath_data):  
    ...
    composed_transforms2 = torchvision.transforms.Compose([ToTensor2(), FixedCrop_gen(shape = (crop_size,crop_size))])
    ...
    if 'natural' in dataset_arg:
        ...
        dataset_test_real = Get_sample_batch(all_files_mat_test, composed_transforms)
...
# 获取三通道的图片
class Get_sample_noise_batch_3(object):
    """Get image from noisy pairs for noise training (gray)"""
    ...
    def __getitem__(self, idx):
        ...
        sample_loaded = scipy.io.loadmat(all_files[idx])
        noisy_im = np.empty((16, 640, 1080, 3))
        
        for i in range(0, 16):
            sample_loaded = scipy.io.loadmat(all_files[idx+i])
            noisy_im[i] = sample_loaded['noisy'][:, :, 0:3]
        ...
        gt_im = np.empty((16, *sample_loaded['noisy'].shape))
        gt_im = np.repeat((gt_loaded['gt'][:, :, 0:3]/gt_loaded['alpha'])[np.newaxis], 16, axis = 0)
...
# 获取自然场景下的三通道图片
class Get_sample_batch_3(object):
    """Loads in real still clean/noisy pairs dataset"""
    ...
    def __getitem__(self, idx):
        ...
        noisy_im = np.empty((16, *sample_loaded['noisy_list'].shape[1:]), dtype = 'float32')
        gt_im = np.empty((16, *sample_loaded['noisy_list'].shape[1:]), dtype = 'float32')
        gt_im = np.repeat(np.mean(sample_loaded['gt_list'][:, :, :, 0:3].astype('float32'), 0)[np.newaxis], 16, axis = 0)
        ...
        if sample_loaded['noisy_list'].shape[0]<16:
            ...
            noisy_im = sample_loaded['noisy_list'][:, :, :, 0:3].astype('float32')[low_ind:low_ind+16]#.copy()
        if alpha:
            gt_im = gt_im/sample_loaded['alpha']
        else:
            noisy_im = noisy_im*sample_loaded['alpha']
...
# 获取3通道的数据    
class Get_sample_batch_video_distributed2_3(object):
    """Loads in images from our clean RGB+NIR video dataset"""
    ...
    def __getitem__(self, im_ind, seq_ind = None):
        ...
        sample = {'gt_label_nobias': noisy_im[:, :, :, :3],}
        if self.transform:
            sample = self.transform(sample)
